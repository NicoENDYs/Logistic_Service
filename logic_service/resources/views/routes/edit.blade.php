        ...
        <form action="{{ route('routes.update', $route) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="destination" id="destination" value="{{ old('destination', $route->destination) }}" 
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="distance_km" id="distance_km" value="{{ old('distance_km', $route->distance_km) }}" 
                ...
                <div>
                    ...
                    <input type="time" name="estimated_time" id="estimated_time" value="{{ old('estimated_time', $route->estimated_time) }}" 
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror" required>
                        <option value="activa" {{ $route->status == 'activa' ? 'selected' : '' }}>Activa</option>
                        <option value="inactiva" {{ $route->status == 'inactiva' ? 'selected' : '' }}>Inactiva</option>
                        <option value="pendiente" {{ $route->status == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
